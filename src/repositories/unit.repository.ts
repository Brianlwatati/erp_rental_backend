...
export async function findUnits(companyId: string, buildingId: string) {
  return (
    await query(
      `SELECT u.* FROM rental_units u JOIN rental_buildings b ON b.id=u.building_id JOIN rental_properties p ON p.id=b.property_id WHERE p.company_id=$1 AND u.building_id=$2 ORDER BY u.unit_number`,
...
export async function updateUnit(companyId: string, id: string, d: any) {
  return (
    (
      await query(
        `UPDATE rental_units u SET unit_type_id=COALESCE($3,u.unit_type_id),unit_number=COALESCE($4,u.unit_number),floor=COALESCE($5,u.floor),monthly_rent=COALESCE($6,u.monthly_rent),deposit_amount=COALESCE($7,u.deposit_amount),status=COALESCE($8,u.status),description=COALESCE($9,u.description),updated_at=NOW() FROM rental_buildings b JOIN rental_properties p ON p.id=b.property_id WHERE u.id=$2 AND u.building_id=b.id AND p.company_id=$1 RETURNING u.*`,
        [
          companyId,
          id,
          d.unitTypeId,
