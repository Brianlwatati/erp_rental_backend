...
export async function findInvoices(
  ...
  if (filters.leaseId) {
    ...
    clauses.push(`lease_id=$${params.length}`);
  }
  return (
    await query(
      `SELECT * FROM rental_invoices WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC`,
      params,
    )
  ).rows;
}

export async function findInvoicesNotFullyPaid(
  companyId: string,
...
export async function findInvoiceById(
  ...
  return (
    (
      await exec(e).query(
        "SELECT * FROM rental_invoices WHERE company_id=$1 AND id=$2",
        [companyId, id],
      )
    ).rows[0] ?? null
  );
...
export async function findInvoiceItems(invoiceId: string, e?: Executor) {
  return (
    await exec(e).query(
      "SELECT * FROM rental_invoice_items WHERE invoice_id=$1 ORDER BY created_at",
      [invoiceId],
    )
  ).rows;
}
...
export async function createInvoiceItem(
  ...
  const amount =
    item.amount ?? Number(item.quantity ?? 1) * Number(item.unitPrice);
  return (
    await e.query(
...
export async function deleteInvoiceItemRow(id: string, e: Executor) {
  return (
    (
      await e.query(
        "DELETE FROM rental_invoice_items WHERE id=$1 RETURNING invoice_id",
        [id],
      )
    ).rows[0] ?? null
  );
}
export async function findInvoiceItemById(
  companyId: string,
  id: string,
  e?: Executor,
) {
...
export async function deleteInvoice(companyId: string, id: string) {
  return (
    (
      await query(
        "DELETE FROM rental_invoices WHERE company_id=$1 AND id=$2 RETURNING id",
        [companyId, id],
      )
    ).rows[0] ?? null
  );
