...
export async function findLeases(companyId: string, filters: { status?: string; tenantId?: string; unitId?: string }) {
  ...
  return (await query(`SELECT * FROM rental_leases WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC`, params)).rows;
}
export async function findLeaseById(companyId: string, id: string) {
  return (await query('SELECT * FROM rental_leases WHERE company_id=$1 AND id=$2', [companyId, id])).rows[0] ?? null;
}
export async function createLease(companyId: string, d: any) {
  return (await query(
    ...
    [companyId, d.unitId, d.tenantId, d.leaseNumber, d.startDate, d.endDate ?? null, d.monthlyRent, d.depositAmount ?? 0, d.billingDay ?? 1, d.status ?? null, d.notes ?? null]
  )).rows[0];
}
export async function updateLease(companyId: string, id: string, d: any) {
  return (await query(
    ...
    [companyId, id, d.leaseNumber, d.startDate, d.endDate, d.monthlyRent, d.depositAmount, d.billingDay, d.status, d.notes]
  )).rows[0] ?? null;
}
...
export async function deleteLease(companyId: string, id: string) {
  return (await query('DELETE FROM rental_leases WHERE company_id=$1 AND id=$2 RETURNING id', [companyId, id])).rows[0] ?? null;
}
