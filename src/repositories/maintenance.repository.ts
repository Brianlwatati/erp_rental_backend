...
export async function findRequests(companyId: string, filters: { status?: string; unitId?: string; propertyId?: string }) {
  const clauses = ['company_id=$1'];
  const params: unknown[] = [companyId];
  if (filters.status) { params.push(filters.status); clauses.push(`status=$${params.length}`); }
  if (filters.unitId) { params.push(filters.unitId); clauses.push(`unit_id=$${params.length}`); }
  if (filters.propertyId) { params.push(filters.propertyId); clauses.push(`property_id=$${params.length}`); }
  return (await query(`SELECT * FROM rental_maintenance_requests WHERE ${clauses.join(' AND ')} ORDER BY reported_at DESC`, params)).rows;
}
export async function findRequestById(companyId: string, id: string, e?: Executor) {
  return (await exec(e).query('SELECT * FROM rental_maintenance_requests WHERE company_id=$1 AND id=$2', [companyId, id])).rows[0] ?? null;
}
export async function createRequest(companyId: string, d: any) {
  return (await query(
    ...
    [companyId, d.propertyId, d.unitId ?? null, d.tenantId ?? null, d.title, d.description ?? null, d.priority ?? null, d.createdBy ?? null]
  )).rows[0];
}
...
export async function setRequestStatus(companyId: string, id: string, status: string, extra: { vendorId?: string | null; resolvedAt?: boolean } = {}) {
  return (await query(
    `UPDATE rental_maintenance_requests SET status=$3,
      ...
      updated_at=NOW()
     WHERE company_id=$1 AND id=$2 RETURNING *`,
    [companyId, id, status, extra.vendorId ?? null, extra.resolvedAt ?? false]
  )).rows[0] ?? null;
}
export async function deleteRequest(companyId: string, id: string) {
  return (await query('DELETE FROM rental_maintenance_requests WHERE company_id=$1 AND id=$2 RETURNING id', [companyId, id])).rows[0] ?? null;
}
...
export async function createCost(requestId: string, d: any, expenseId: string | null, e: Executor) {
