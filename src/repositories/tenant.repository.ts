...
export async function findTenants(companyId: string, status?: string) {
  if (status)
    return (
      await query(
        "SELECT * FROM rental_tenants WHERE company_id=$1 AND status=$2 ORDER BY created_at DESC",
        [companyId, status],
      )
    ).rows;
  return (
    await query(
      "SELECT * FROM rental_tenants WHERE company_id=$1 ORDER BY created_at DESC",
      [companyId],
    )
  ).rows;
}
export async function findTenantById(companyId: string, id: string) {
  return (
    (
      await query(
        "SELECT * FROM rental_tenants WHERE company_id=$1 AND id=$2",
        [companyId, id],
      )
    ).rows[0] ?? null
  );
...
export async function deleteTenant(companyId: string, id: string) {
  return (
    (
      await query(
        "DELETE FROM rental_tenants WHERE company_id=$1 AND id=$2 RETURNING id",
        [companyId, id],
      )
    ).rows[0] ?? null
  );
