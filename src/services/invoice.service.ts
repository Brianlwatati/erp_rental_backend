...
async function verifyLeaseAndTenant(c: string, leaseId: string, tenantId: string) {
  const lease = (await query('SELECT id, tenant_id FROM rental_leases WHERE id=$1 AND company_id=$2', [leaseId, c])).rows[0];
  if (!lease) throw new Error('LEASE_NOT_FOUND');
  const tenant = (await query('SELECT id FROM rental_tenants WHERE id=$1 AND company_id=$2', [tenantId, c])).rows[0];
  if (!tenant) throw new Error('TENANT_NOT_FOUND');
  if (lease.tenant_id !== tenantId) throw new Error('LEASE_TENANT_MISMATCH');
}

export const listInvoices = (c: string, filters: { status?: string; tenantId?: string; leaseId?: string }) => repo.findInvoices(c, filters);
...
export async function createInvoice(c: string, d: any) {
  ...
  return withTransaction(async (client) => {
    const invoice = await repo.createInvoiceHeader(c, { ...d, invoiceNumber: d.invoiceNumber ?? generateDocumentNumber('INV') }, client);
