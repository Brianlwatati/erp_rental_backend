...
export const listPayments = (c: string, filters: { status?: string; tenantId?: string }) => repo.findPayments(c, filters);
...
async function applyAllocation(c: string, tenantId: string, paymentId: string, invoiceId: string, amount: number, client: any) {
  const invoice = await invoiceRepo.findInvoiceById(c, invoiceId, client);
  if (!invoice) throw new Error('INVOICE_NOT_FOUND');
  if (invoice.tenant_id !== tenantId) throw new Error('INVOICE_TENANT_MISMATCH');
  if (!['ISSUED', 'PARTIALLY_PAID', 'OVERDUE'].includes(invoice.status)) throw new Error('INVOICE_NOT_PAYABLE');
  if (amount > Number(invoice.balance)) throw new Error('ALLOCATION_EXCEEDS_BALANCE');
...
export async function createPayment(c: string, d: any) {
  ...
  return withTransaction(async (client) => {
    const payment = await repo.createPayment(c, { ...d, paymentNumber: d.paymentNumber ?? generateDocumentNumber('PMT') }, client);
    for (const a of allocations) {
      await applyAllocation(c, d.tenantId, payment.id, a.invoiceId, a.amount, client);
    }
...
export async function allocatePayment(c: string, paymentId: string, d: { invoiceId: string; amount: number }) {
  return withTransaction(async (client) => {
    ...
    if (alreadyAllocated + d.amount > Number(payment.amount)) throw new Error('ALLOCATION_EXCEEDS_PAYMENT');
    await applyAllocation(c, payment.tenant_id, paymentId, d.invoiceId, d.amount, client);
...
export async function reversePayment(c: string, id: string) {
  return withTransaction(async (client) => {
    ...
    for (const a of allocations) {
      await invoiceRepo.adjustInvoiceAmountPaid(a.invoice_id, -Number(a.amount), client);
...
export async function issueReceipt(c: string, paymentId: string, d: any) {
  return withTransaction(async (client) => {
    ...
    return repo.createReceipt(c, paymentId, Number(payment.amount), { ...d, receiptNumber: d.receiptNumber ?? generateDocumentNumber('RCT') }, client);
  });
}
